...
在 Unix Shell 脚本中，我们可以添加以 开头的一行，#!告诉 Unix 应该使用哪个可执行文件来运行该脚本。这两个字符有多个名称，包括hashbang、sharp-exclamation、sha-bang（“sha” 代表“sharp”）和shebang。否则，大多数 Shell 脚本语言会将 hashbang 行视为注释，JavaScript 也是如此。以下是 Node.js 中常见的 hashbang 行：

#!/usr/bin/env node
如果我们想向传递参数node，我们必须使用env选项-S（为了安全起见，有些 Unix 不需要它）：

#!/usr/bin/env -S node --enable-source-maps --no-warnings=ExperimentalWarning

## 6.26

1. 对于AI程序，我们发现，模型定义非常简单，一共也就20行代码。训练代码也很少，不超过100行。它和传统的程序最大的区别在哪呢？
...
1. https://github.com/angular/web-codegen-scorer 谷歌angular团队开发的AI网站评分
2. https://navcat.dev/docs/ 3D threejs 导航库
3. viteconf 相关内容 介绍了 vite+ / vite devtools 相关内容 / oxlint 相关内容 / Nitro v3 SRR系统 / vitest 4

## 12.03

1. https://github.com/tw93/mole 像鼹鼠一样深入挖掘，优化你的Mac电脑。
