...
检查设计布局时，请考虑技术限制和功能。专注于尝试识别可能的问题或昂贵的设计解决方案，并尽快发出警告
使用设计系统来实现一致性和更好的可维护性
实现新功能时验证不同的用户场景，注重使用的便捷性
考虑考虑所有界面状态
考虑视觉层次结构和分组，并仔细检查填充和边距。应该和设计中的一样
注意布局中体现对比的细节：字体大小和样式、颜色和尺寸。对于可访问性也很重要
在实现设计布局时检查元素的对齐情况：不一致的对齐方式使界面看起来很草率
...
小程序发版，有覆盖过程
低打开率的场景，访问量上涨，比如广告等等
性能真的有问题，加载耗时变长，用户没耐心等